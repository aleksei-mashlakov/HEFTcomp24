...
from src.utils import preprocess_with_coord_averaging
...
class BaseDataProcessor(abc.ABC):
    ...
    def submit_transform(
        self, forecast: pd.DataFrame, market_bid: pd.DataFrame
    ) -> dict[str, Any]:
...
class MeanWeatherDataProcessor(BaseDataProcessor):
    def _transform_solar_data(self, data) -> xr.Dataset:
        ...
        latest_solar_features = preprocess_with_coord_averaging(
            dataset=latest_dwd_solar, dims="point", features=SOLAR_FEATURES
        )
        latest_solar_features.rename(
            columns={"Temperature": "SolarTemperature"}, inplace=True
        )
    ...
    def _transform_wind_data(self, data: pd.DataFrame) -> xr.Dataset:
        ...
        latest_wind_data_features = preprocess_with_coord_averaging(
    ...
    def transform(self, data: InputData) -> pd.DataFrame:
        ...
        datetime = pd.to_datetime("today").strftime("%Y-%m-%d")
        latest_forecast_table.to_csv(
            f"./data/raw/online/latest_forecast_table_{datetime}.csv", index=False
        )
